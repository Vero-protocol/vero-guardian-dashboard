...
export async function requestFreighterPublicKey(): Promise<string> {
  if (typeof freighterClient.requestAccess === 'function') {
    try {
      ...
      // Re-throw already-normalized messages; wrap everything else.
      if (err instanceof Error && err.message.startsWith('Freighter')) {
        throw err;
      }
      throw new Error(
        getWalletErrorMessage(err, 'Freighter could not grant wallet access. Open Freighter and try again.')
      );
    }
  }
  ...
  if (typeof freighterClient.getPublicKey === 'function') {
    try {
      ...
      throw new Error(getWalletErrorMessage(err, 'Freighter could not return a public key. Open Freighter and try again.'));
    }
  }
...
export async function readCurrentFreighterPublicKey(): Promise<string | null> {
  ...
  if (
    ...
    try {
      ...
      throw new Error(getWalletErrorMessage(err, 'Unable to read Freighter wallet state.'));
    }
  }
  ...
  if (typeof freighterClient.getPublicKey === 'function') {
    try {
      ...
      throw new Error(getWalletErrorMessage(err, 'Unable to read Freighter public key.'));
    }
  }
...
export const freighterProvider: StellarWalletProvider = {
  ...
  signTransaction: async (xdr: string, opts?: { networkPassphrase?: string }) => {
    ...
    try {
      ...
      throw new Error(getWalletErrorMessage(err, 'Freighter failed to sign the transaction'));
    }
  },
};
